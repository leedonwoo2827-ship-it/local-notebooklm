...
│  ─ PDF          │                       │  📊 PPTX 보고서       │
│  ─ Docx         │  사용자: 핵심은?      │  📋 슬라이드 교안(텍스트) │
│  ─ HWPX         │  AI: ...[Paper p.3]   │  🖼 카드뉴스          │
│  ─ SRT/VTT      │                       │  🧠 마인드맵          │
│  ─ MP4 (STT)    │  [질문 입력]          │  🗂 플래시카드(.apkg/.xlsx) │
│  노트북 선택 ▾  │                       │  ❓ 퀴즈(.xlsx)       │
│                 │                       │  생성된 메모 ⬇ 🗑     │
└─────────────────┴───────────────────────┴───────────────────────┘
```
...
- **Studio**: 버튼 한 번에 산출물 생성. 디스크에 영구 저장되어 앱 재시작 후에도 우측에 그대로 표시. 다운로드 파일이 있는 항목은 헤더에 `⬇` 마커, 좌측 `🗑` 으로 두 번 클릭 삭제.
  - **PPTX 보고서** = 발표/임원 보고용 6장 요약. **슬라이드 교안(텍스트)** = 강의용 20장 분량 .md (NotebookLM·GPT·Gemini Canvas 에 붙여넣어 가공).
  - **퀴즈/플래시카드** 는 `.xlsx` 로도 다운로드 (플래시카드는 Anki `.apkg` 도 같이).
