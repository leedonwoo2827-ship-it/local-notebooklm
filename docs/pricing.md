...
**산출물별 회피 상태**:
- ✅ **슬라이드 교안 / PPTX 보고서** — `studio/slides.py` 의 `make_slides_lite()`
  가 N≥10 시 자동으로 4장씩 그룹 호출 후 병합 (`CHUNK_THRESHOLD=10`,
  `GROUP_SIZE=4`). 20~30장도 60초 천장 안전.
- ⚠️ **HWPX 보고서, 종합 카드뉴스, 마인드맵 등** — 아직 단발 호출. 입력+출력
  합이 약 **5만 토큰 이상** 이면 408 위험. 같은 청크 분할 패턴을 적용하는 게
  다음 라운드 작업.

**단발 호출 산출물 회피 가이드**:
- 자막은 노트북당 1~2개씩. 책은 1장(약 20쪽) 단위.
...
퀴즈 5문항 + 슬라이드 교안 20장(분할 호출) + 플래시카드 30장
...
| 슬라이드 교안 20장 (5회 분할) | 80,000 | 14,000 | claude-sonnet-4-6 |
...
| 슬라이드 교안 (분할 5회) | $0.2400 | $0.2100 | **$0.450** |
| 플래시카드 | $0.0480 | $0.0600 | **$0.108** |
| 채팅 5회 | $0.0050 | $0.0050 | **$0.010** |
| **합계 (1장 풀세트)** | | | **약 $0.67** ≈ 900원 |

> 인덱싱은 노트북당 1회. 같은 책의 다음 장은 새 노트북이라 다시 인덱싱하지만
> 책 1장 단위면 인덱싱 비용 자체가 미미.

> **분할 호출 비용 패턴**: 슬라이드 교안 20장은 4장씩 5회 호출, 매 호출마다
> 같은 RAG 자료가 입력으로 재전송되어 입력 토큰이 약 5배가 된다. 그래도 풀세트
> 1,000원 미만. 8장으로 줄이면(2회 호출) 슬라이드 교안 비용이 ~$0.18 로 떨어져
> 풀세트 약 500원.
...
> 슬라이드 교안 20장은 분할 호출로 안전. HWPX 보고서는 단발 호출이라 자막 3개
> 합치면 60초 천장에 걸릴 수 있어 회차별로 노트북을 분리하는 게 안전.
